...
common_features = intersect(intersect(colnames(mutation), colnames(cna)),colnames(rna))
...
start_time <- Sys.time()
fit = iClusterBayes(dt1=mutation, dt2=cna, dt3=rna, dt4=protein, type = c("gaussian","gaussian","gaussian", "gaussian"), K=30)
end_time <- Sys.time()
end_time - start_time
